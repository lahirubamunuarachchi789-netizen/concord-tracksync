...
/**
 * Build a delta scan-row fetcher: fetch only rows created AFTER `since`.
 * Used by the Live Dashboard's smart-polling to avoid re-fetching the
 * full day/week window on every 30s tick - only newly inserted scans
 * are returned, dramatically reducing Supabase egress for factory TVs
 * that run continuously.
 *
 * @param {object} supabaseClient supabase-js-like client (injectable for tests)
 */
export function createDeltaScanRowsFetcher(supabaseClient) {
  return async function fetchDeltaScanRows({ departmentId, since }) {
    let query = supabaseClient
      .from(DATA_UPDATES_TABLE)
      .select(DASH_SCAN_COLUMNS)
      .gt('created_at', since)
...
const fetchDeltaScanRows = createDeltaScanRowsFetcher(supabase);
...
/**
 * Delta fetch: retrieve only scan rows created AFTER `since` for one
 * department. Used by the Live Dashboard's smart-polling to avoid
 * re-fetching the full day/week window on every 30s tick.
 *
 * @param {object} params
 * @param {string} params.departmentId department name
 * @param {string} params.since       ISO timestamp - return rows strictly after this
 * @returns {Promise<object[]>} newly created scan rows (created_at, count, qc_status)
 */
export function fetchDashboardDelta({ departmentId, since }) {
  if (!since) throw new Error('A since timestamp is required for delta fetch.');
  return fetchDeltaScanRows({ departmentId, since });
}
  ...
  return {
    ...
    weekAchievement:
      weekPlanQty > 0
        ...
        : 0,
    // Raw week scan rows for delta tracking. The component uses these to
    // merge newly fetched delta rows without re-querying the full week window.
    _rawWeekRows: weekRows,
  };
}
