 ...
 *     (planed_qty / planed_hour). Past the planned hours the target keeps
 *     counting up (overtime signal) while `ratio` caps at the finish line.
...
export function computeGpsTarget({ slstTimeHHmm, plannedQty, plannedHours }) {
  ...
  return {
    qty: Math.max(linearRatio * qty, 0),
    ratio: linearRatio,
