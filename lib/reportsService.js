...
export async function buildPoSummary(po) {
  ...
  // WIP is calculated between consecutive sequence levels using combined Out Totals
  // WIP = (Combined Out Total of preceding sequence) - (Combined Out Total of current sequence)

  // First pass: Group departments by sequence and calculate combined Out Totals per sequence
  ...
  for (const dept of departments) {
    ...
    const combinedOutTotal = {};
    for (const size of sizes) {
      combinedOutTotal[size] = sizeMetrics[size]?.outTotal || 0;
    }

    if (sequenceGroups.length === 0 || sequenceGroups[sequenceGroups.length - 1].sequence !== dept.sequence) {
      sequenceGroups.push({
        sequence: dept.sequence,
        departments: [dept],
        combinedOutTotal: combinedOutTotal,
        combinedOutTotalSum: totalMetrics.outTotal,
        deptNames: [deptName],
      });
    } else {
      const currentGroup = sequenceGroups[sequenceGroups.length - 1];
      currentGroup.departments.push(dept);
      for (const size of sizes) {
        currentGroup.combinedOutTotal[size] = (currentGroup.combinedOutTotal[size] || 0) + (sizeMetrics[size]?.outTotal || 0);
      }
      currentGroup.combinedOutTotalSum += totalMetrics.outTotal;
      currentGroup.deptNames.push(deptName);
    }
  }

  // Second pass: Build rows with WIP calculation between sequences
  let prevSeqCombinedOutTotal = null;
  let prevSeqGroup = null;

  for (const seqGroup of sequenceGroups) {
    const isFirstSequenceGroup = prevSeqCombinedOutTotal === null;

    // Iterate through all departments in this sequence group
    for (const dept of seqGroup.departments) {
      // Safety check: skip if department is undefined
      if (!dept) continue;
      ...
      rows.push({
        ...
        wip: {},
        wipTotal: 0,
        wipFromSequence: null,
      });
    }

    // Calculate WIP for all departments in this sequence group
    // WIP = preceding sequence combined Out Total - current sequence combined Out Total
    // Display WIP on all departments in multi-department target sequences
    if (!isFirstSequenceGroup && seqGroup.departments.length > 0) {
      const wipBySize = {};
      for (const size of sizes) {
        const prevOut = prevSeqCombinedOutTotal[size] || 0;
        const currOut = seqGroup.combinedOutTotal[size] || 0;
        wipBySize[size] = prevOut - currOut;
      }
      const wipTotal = Object.values(wipBySize).reduce((sum, v) => sum + v, 0);

      // Create group label from preceding sequence department names
      const wipLabel = prevSeqGroup.deptNames && prevSeqGroup.deptNames.length > 1
        ? `from ${prevSeqGroup.deptNames.join(', ')}`
        : `from ${prevSeqGroup.deptNames?.[0] || ''}`;

      // Set WIP on all departments in this sequence group
      const startIndex = rows.length - seqGroup.departments.length;
      for (let i = 0; i < seqGroup.departments.length; i++) {
        const row = rows[startIndex + i];
        row.wip = wipBySize;
        row.wipTotal = wipTotal;
        row.wipFromSequence = wipLabel;
      }
    }

    prevSeqCombinedOutTotal = seqGroup.combinedOutTotal;
    prevSeqGroup = seqGroup;
  }
