...
export async function buildPoSummary(po) {
  ...
  // WIP is only calculated between departments with DIFFERENT sequence numbers
  let prevSequenceOutTotal = null;
  let prevSequenceLabel = null;
  let prevSequence = null;
  let prevSequenceDeptNames = [];
  ...
  for (const dept of departments) {
    ...
    // Calculate WIP only between departments with DIFFERENT sequence numbers
    // Skip WIP calculation for departments sharing the same sequence
    const currentOutTotal = totalMetrics.outTotal;
    let wipBySize = {};
    let wipTotal = 0;
    let wipLabel = null;

    if (prevSequenceOutTotal !== null && prevSequence !== dept.sequence) {
      for (const size of sizes) {
      ...
      // Create group label from all preceding sequence department names
      wipLabel = prevSequenceDeptNames.length > 1
        ? `from ${prevSequenceDeptNames.join(', ')}`
        : `from ${prevSequenceDeptNames[0] || prevSequenceLabel}`;
    }
    ...
    rows.push({
      ...
      wip: wipBySize,
      wipTotal,
      wipFromSequence: prevSequence !== dept.sequence ? wipLabel : null,
    });

    // Store current Out Total for next sequence WIP calculation
    // Track all department names in the current sequence for group labeling
    if (prevSequence !== dept.sequence) {
      prevSequenceOutTotal = {};
      for (const size of sizes) {
        prevSequenceOutTotal[size] = sizeMetrics[size]?.outTotal || 0;
      }
      prevSequenceLabel = deptName;
      prevSequenceDeptNames = [deptName];
      prevSequence = dept.sequence;
    } else {
      // Same sequence: accumulate Out Total for WIP calculation across same-sequence departments
      for (const size of sizes) {
        prevSequenceOutTotal[size] = (prevSequenceOutTotal[size] || 0) + (sizeMetrics[size]?.outTotal || 0);
      }
      prevSequenceDeptNames.push(deptName);
    }
  }
