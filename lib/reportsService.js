...
export async function buildPoSummary(po) {
  ...
  for (const seqGroup of sequenceGroups) {
    ...
    // Calculate WIP for the first department in this sequence group
    ...
    if (!isFirstSequenceGroup && seqGroup.departments.length > 0) {
      const firstDeptRow = rows[rows.length - seqGroup.departments.length];
      const wipBySize = {};
      for (const size of sizes) {
      ...
      firstDeptRow.wip = wipBySize;
      firstDeptRow.wipTotal = wipTotal;
      firstDeptRow.wipFromSequence = wipLabel;
    }
