...
//
// msk LIFECYCLE STATUS GATE: when the scanned QR EXISTS in the msk
// table it may ONLY be activated when its row is in the 'Packed'
// lifecycle status (set automatically when the shoe's Packing net
// count hits +1). 'Active' - the default status of an un-activated
// floor mapping - and every other status BLOCK the activation
// (evaluateActivationStatus in this module, enforced by
// createActivation in qrActivationService.js). A QR with NO msk row
// at all is ALLOWED through (checkActivationMskStatus skips the gate
 ...
 * status - identical to buildStandardTransactionPayload. `createdAt`
 * optionally overrides the scan timestamp (null = current system time).
 */
...
/* ====================================================================
 * msk lifecycle status gate (existing rows require 'Packed')
 *
 ...
 * count hits +1 (automatic Packing trigger). When a scanned QR EXISTS
 * in msk, activation is allowed ONLY for 'Packed' rows - 'Active' and
 * every other status block the scan, so a packed shoe can be activated
 * exactly once. A QR with NO msk row at all is NOT blocked: the gate
 * lets it through and activation proceeds normally.
 * ==================================================================== */
...
/**
 * Pure msk status gate for the QR Activation flow: an msk row whose
 * status is 'Packed' allows the activation; 'Active' and every other
 * status block it. A blank/unknown status VALUE also blocks here as a
 * defensive fail-safe - note that a QR with NO msk row at all never
 * reaches this function: checkActivationMskStatus (qrActivationService.js)
 * allows it through BEFORE calling this gate.
 *
