...
/**
 * Resolve the `created_at` value for a data_updates record.
 *
 * A manual date & time override (anything `new Date(value)` accepts -
 * an ISO-8601 string or a Date instance) is stored verbatim as an ISO
 * instant. When no override is given - or the value is blank / not
 * parseable - the CURRENT system timestamp is used, so a stale or
 * malformed manual value can never break a record: it always receives
 * a valid `created_at`.
 *
 * @param {string|Date|null|undefined} provided manual override
 * @returns {string} ISO-8601 UTC instant
 */
export function resolveScanTimestamp(provided) {
 ...
 * CURRENT system time (new Date()) is used instead.
 */
export function buildStandardTransactionPayload({
  ...
  return {
    ...
    created_at: resolveScanTimestamp(createdAt), // scan timestamp (NOW unless overridden)
  };
}
...
/** V3 failure message (exact) - kept for QRs with nothing comparable. */
export const BLOCK_SIZE_MISMATCH =
  'Size Mismatch: Inner Box Size does not match Shoe Size';

/**
 * V3 failure (a): NO row was returned from srl_num for the extracted
 * Box Code. Dynamic so the exact code is included for debugging.
 */
export function srlNumNotFoundReason(boxCode) {
  return `Box code ${boxCode} not found in srl_num database`;
}

/**
 * V3 failure (b): both sizes exist but differ. Dynamic so BOTH sizes
 * are included for debugging (srl_num size first, shoe size second).
 */
export function sizeMismatchReason(dbSize, shoeSize) {
  return `Size Mismatch: Inner Box Size ('${dbSize}') does not match Shoe Size ('${shoeSize}')`;
}
 ...
 * separators are skipped while scanning). The result is trimmed and
 * ready for the srl_num box_num query. Returns null when the QR does
 * not carry 16 leading digits.
 */
export function extractInnerBoxCode(innerQr) {
  const text = String(innerQr || '').trim();
  let skipped = 0;
  let collected = '';
  ...
  return collected.length === 13 ? collected.trim() : null;
}
...
/**
 * Normalize a size for comparison: numeric equality when both sides
 * are numeric ('35.0' and '035' both normalize to '35'), otherwise the
 * trimmed UPPERCASED string (required case-insensitive comparison).
 */
...
/**
 * V3 comparison - flexible by requirement:
 *   String(dbSize).trim().toUpperCase() === String(shoeSize).trim().toUpperCase()
 * with an extra numeric-equality tolerance so '35.0' / '035' still
 * match '35' (same real-world size, different storage formats).
 */
export function sizesMatch(a, b) {
  if (a == null || b == null) return false;
  if (String(a).trim().toUpperCase() === String(b).trim().toUpperCase()) {
    return true;
  }
...
export function validateDualScanPair({ innerQr, orgQr, srlSize }) {
  ...
  // V3: the srl_num size must match the shoe size, with DISTINCT
  // failures: (a) no srl_num row -> the exact box code is named;
  // (b) both sizes present but different -> both sizes are named.
  // The static BLOCK_SIZE_MISMATCH only remains for QRs that do not
  // even carry a 13-digit Box Code or a shoe size segment.
  const boxCode = extractInnerBoxCode(innerQr);
  const shoeSize = parseOrgQr(orgQr).size;
  if (!boxCode || shoeSize == null) {
    // Nothing to compare: the label has no 13-digit Box Code or the
    // shoe org_qr carries no size segment.
    return { reason: BLOCK_SIZE_MISMATCH };
  }
  if (srlSize == null) {
    // NO row returned from srl_num for this Box Code (a row without a
    // size cannot prove a match either) - nothing is written.
    return { reason: srlNumNotFoundReason(boxCode) };
  }
  if (!sizesMatch(srlSize, shoeSize)) {
    return { reason: sizeMismatchReason(srlSize, shoeSize) };
  }
  return null;
}