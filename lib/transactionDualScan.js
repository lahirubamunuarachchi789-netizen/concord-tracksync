 ...
 *       substring.
 *   V2. PO match     - the inner PO code (backwards extraction) must
 *       equal the shoe org_qr PO code (hyphen-aware last-4 extraction).
...
/**
 * V2 (inner side): moving BACKWARDS from the end of the inner QR,
 * skip the first block of 8 digits, then extract the preceding 4
 * digits (non-digit URL separators are skipped while scanning).
 * Returns the 4-digit code, or null when there are not enough digits.
 */
export function extractInnerBoxPo(innerQr) {
  const text = String(innerQr || '');
  const collected = [];
  for (let i = text.length - 1; i >= 0 && collected.length < 12; i -= 1) {
    if (isDigitChar(text[i])) collected.push(text[i]);
  }
  if (collected.length < 12) return null;
  return collected.slice(8, 12).reverse().join('');
}
