...
//      BAN GUARD: a row explicitly marked 'ban' blocks the scan with
//      the dedicated banned message FIRST - in the scanned msk_qr
//      mode (the ban check reads the same Rule 1 rows) and in the
//      Packing single-scan lookup mode (the resolved org_qr's msk
//      status is verified directly; an unreachable msk table blocks
//      fail-safe). Banned QR codes can never be entered.
...
export function createSupabaseGuardDb(supabase) {
  return {
    ...
    /**
     * ALL msk rows carrying this org_qr (status only) - the Ban Guard
     * check for the Packing single-scan lookup mode, where Rule 1 is
     * skipped and the org_qr was resolved from data_updates instead.
     * Throws when the table cannot be reached - the guard blocks
     * fail-safe (the ban status cannot be proven).
     */
 ...
 *        data_updates - and ONLY Rules 2-5 run on this org_qr. The
 *        Ban Guard still verifies the org_qr's msk status first: a
 *        banned QR blocks in Packing lookup mode as well.
...
export async function validateStandardScan({ scannedQr, user, db, innerQr = null, qcStatus = null, orgQr = null }) {
  ...
  /* ---- Rule 1: Active-status MSK gate + Ban Guard ----------------- */
  // If an org_qr is pre-resolved (e.g., from data_updates for Packing mode),
  // skip the msk lookup entirely - the Ban Guard then verifies the
  // resolved org_qr's msk status directly (Packing branch below).
  ...
  if (!orgQr) {
    let rows = null;
    try {
      rows = await db.listMskRowsByMskQr(scanned);
    } catch {
      // Table unreachable (network / RLS / missing): no active mapping
      // can be proven, so the scan is blocked under Rule 1.
      mskOffline = true;
    }
    // BAN GUARD (part of Rule 1): a row explicitly marked 'ban' blocks
    // the scan outright - the QR is frozen and may never be processed.
    // The check runs BEFORE the active pick so a banned row gets the
    // exact banned message instead of the generic no-mapping text,
    // and it wins over any other legacy row that would still resolve.
    if (rows && pickBannedMskRow(rows)) {
      return { ok: false, reason: BLOCK_QR_BANNED_TRANSACTION };
    }
    const active = rows ? pickActiveMskRow(rows) : null;
    resolvedOrgQr = active?.org_qr || null;
    if (!resolvedOrgQr) {
      return { ok: false, reason: BLOCK_NO_ACTIVE_MSK, offline: mskOffline };
    }
  } else {
    // Packing single-scan lookup mode: Rule 1 (incl. the msk_qr lookup)
    // was skipped, so the Ban Guard verifies the resolved org_qr's msk
    // status DIRECTLY. A banned QR must never be entered through the
    // Inner Box lookup path either. Fail-safe: an unreachable msk table
    // blocks the scan (the ban status cannot be proven).
    let statusRows = null;
    try {
      statusRows = await db.listMskStatusesByOrgQr(resolvedOrgQr);
    } catch {
      return { ok: false, reason: BLOCK_BAN_STATUS_UNREACHABLE };
    }
