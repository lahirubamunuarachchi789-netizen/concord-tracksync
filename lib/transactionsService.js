...
//    gate + QR BAN guard + preceding/current/parallel sequence net
//    count guards + downstream department sequence guard + PO+Size
//    cut_qty limit).
//  * data_updates : standard transaction records (qr_code =
//    the resolved org_qr), with a localStorage queue fallback
//    so scanning never blocks the production floor. The flush
//    re-checks the msk ban status: a queued record whose org_qr
//    was banned while offline is dropped, never synced.
...
import { pickBannedMskRow } from './qrBanGuard.js';
 ...
 * statuses / non-Finishing departments). `createdAt` optionally
 * overrides the scan timestamp (null = current system time).
 */
 ...
 * `createdAt` optionally overrides the scan timestamp; when null the
 * CURRENT system time is used (fallback - the toggle-off default).
...
export async function createTransaction(
  ...
  const payload = buildTransactionPayload(user, orgQr, recordStatus, qcStatus, innerQr, createdAt);
  ...
  try {
    ...
    // primary key so the Recent Transactions bin icon can target the
    // EXACT database record for deletion.
    ...
    result = {
      ok: true,
      status: 'synced',
      row: { ...payload, id: inserted?.[0]?.id ?? null, client_ref: clientRef },
    };
  } catch (err) {
...
export async function retryQueuedTransactions() {
  ...
  for (const entry of queue) {
    ...
    // BAN GUARD at sync time: a QR banned while its record sat in the
    // offline queue must never be written to data_updates. The msk row
    // is addressed by org_qr (= data_updates.qr_code). A banned QR is
    // DROPPED; an unprovable ban status (msk unreachable) KEEPS the
    // entry queued for a later pass - fail-safe, the same policy the
    // scan guards apply.
    if (payload?.qr_code) {
      try {
        const statusRows = await guardDb.listMskStatusesByOrgQr(payload.qr_code);
        if (pickBannedMskRow(statusRows)) {
          dropped += 1;
          continue;
        }
      } catch {
...
/**
 * Remove one pending offline transaction from the local queue so a
 * deleted scan can never be re-synced into data_updates afterwards.
 * @returns {number} how many pending queue entries were removed
 */
export function removeQueuedTransaction(clientRef) {
  if (!clientRef) return 0;
  const remaining = readQueue().filter((entry) => entry?.client_ref !== clientRef);
...
/**
 * Delete ONE standard transaction record from the data_updates table
 * (the bin icon on the Recent Transactions log of the Standard
 * Transactions page).
 *
 * Row targeting:
 *   - `id` primary key when the UI row carries one (rows saved by
 *     createTransaction capture it via INSERT ... RETURNING id),
 *   - otherwise the exact composite of every transaction field -
 *     `created_at` is the client-side scan timestamp set by
 *     buildStandardTransactionPayload, which makes the combination
 *     unique per scan (nulls become `is.null` filters through .match).
 *
 * The delete is VERIFIED: `.select('id')` makes the DELETE return the
 * removed rows, so BOTH a database error AND a silent no-op are
 * reported as an explicit failure instead of pretending success.
 * PostgREST resolves an RLS-blocked or not-found DELETE as HTTP 200
 * with an EMPTY array (`error` stays null!) - without this check the
 * UI would remove the row while the database record survives. A record
 * that only ever lived in the offline queue (never synced) is deleted
 * by removing its pending queue entry.
 *
 * Never throws - returns { ok: true } | { ok: false, error } so the
 * view decides between UI cleanup and the centered ErrorModal.
 *
 * @param {object} row the history row to delete (payload fields with an
 *        optional `id` / `client_ref`)
 * @param {object} [client] Supabase client (injectable for tests)
 */
export async function deleteTransactionRecord(row, client = supabase) {
  try {
    const query =
      row?.id != null
    ...
    const dbDeleted = deletedRows?.length ?? 0;

    // A pending offline copy of this scan must not re-sync after deletion.
    const queueRemoved = removeQueuedTransaction(row?.client_ref);

    if (dbDeleted === 0 && queueRemoved === 0) {
      // Nothing was deleted anywhere: the record does not exist in the
      // database and no pending offline copy is waiting. The most
      // common cause is a DELETE silently blocked by Row Level
      // Security (missing DELETE policy) - say so explicitly.
      return {
        ok: false,
        error:
          'No matching data_updates record was deleted. The record may have already been removed, or the database is missing the data_updates DELETE policy (run supabase/transactions-schema.sql in the Supabase SQL Editor).',
      };
    }
    ...
    return {
      ok: false,
      error: `Could not delete the transaction from data_updates: ${
