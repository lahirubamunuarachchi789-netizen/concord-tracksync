 ...
 * returns { ok: false } - `reason` carries the exact toast text.
 *
 * @returns {Promise<{ok: true, orgQr: string, departmentContext: object} |
 *                    {ok: false, reason: string, offline?: boolean}>}
 */
export function validateStandardTransactionScan(scannedQr, user) {
  return validateStandardScan({ scannedQr, user, db: guardDb });
}
 ...
 */
export function buildTransactionPayload(user, orgQr, recordStatus, qcStatus) {
  return {
    qr_code: orgQr,
 ...
 * resolved from the msk mapping (lookupOrgQr must run first).
 * @returns {Promise<{ok: boolean, status: 'synced'|'queued'|'failed', row: object, error?: string}>}
 */
export async function createTransaction(user, orgQr, recordStatus, qcStatus) {
  const payload = buildTransactionPayload(user, orgQr, recordStatus, qcStatus);
...
export async function getRecentTransactions(limit = 25) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('qr_code, record_status, qc_status, department, count, created_by, created_at')
