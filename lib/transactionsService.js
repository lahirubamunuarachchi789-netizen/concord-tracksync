...
export function removeQueuedTransaction(clientRef) {
  if (!clientRef) return;
  writeQueue(readQueue().filter((entry) => entry?.client_ref !== clientRef));
}
 ...
 * Transactions page).
 *
 * The row is addressed by its `id` primary key when the UI row carries
 * one, otherwise by the exact composite of every transaction field -
 * `created_at` is the client-side scan timestamp set by
 * buildStandardTransactionPayload, which makes the combination unique
 * per scan (nulls become `is.null` filters through .match).
 *
 * A still-pending offline copy of the scan is also removed from the
 * localStorage queue so it can never be re-synced after deletion.
 *
...
export async function deleteTransactionRecord(row, client = supabase) {
  try {
    if (row?.id != null) {
      ...
      // In-session rows are the createTransaction payload echo - they
    ...
    removeQueuedTransaction(row?.client_ref);
    return { ok: true };
  } catch (err) {
    return {
      ok: false,
      error: String(err?.message || err || 'The transaction could not be deleted.'),
    };
  }
}