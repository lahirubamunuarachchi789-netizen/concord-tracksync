...
//  * purchase_orders : PO list management (fetch / add / delete)
//    on the Supabase table `purchase_orders` (column `po`).
...
export async function fetchPurchaseOrders() {
  ...
  return (data || []).map((r) => String(r.po)).filter(Boolean);
}
...
export async function addPurchaseOrder(po) {
  ...
  try {
    const { error } = await supabase.from(PURCHASE_ORDERS_TABLE).insert([{ po: value }]);
    if (error) throw error;
...
/**
 * Persist one activation (qr_value + PO + size).
 * @returns {Promise<{ok: boolean, status: 'synced'|'queued'|'failed', row: object, error?: string}>}
 */
export async function createActivation(user, qrValue, po, size) {
  const row = {
    qr_value: qrValue,
    po,
    size,
...
export async function getRecentActivations(limit = 25) {
  ...
  try {
    const { data, error } = await supabase
      .from(QR_ACTIVATIONS_TABLE)
      .select('qr_value, po, size, username, department, client_ref, created_at')
