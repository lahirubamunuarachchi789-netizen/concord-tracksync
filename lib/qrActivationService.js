...
import { isFinishingDepartment } from '@/lib/transactionDualScan';
import {
  resolveNextDepartments,
  evaluateDownstreamGuard,
  BLOCK_DOWNSTREAM_DEPT,
} from '@/lib/activationDeptGuard';
 ...
 */
export async function clearInnerQrForQrCode(qrCode) {
  const { error } = await supabase
...
export async function createActivation(user, qrValue, po, size, recordStatus, qcStatus, innerQr = null) {
  ...
  if (qcStatus === 'Return' && innerQr && isFinishingDepartment(user?.department)) {
    await clearInnerQrForQrCode(qrCode);
