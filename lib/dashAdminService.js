...
export function normalizeDashRow(row) {
  const planedHour = Number(row?.planed_hour);
  return {
    id: row?.id ?? null,
    date: String(row?.date ?? '').slice(0, 10),
    department: String(row?.department ?? ''),
    planed_qty: Number(row?.planed_qty) || 0,
    planed_hour: Number.isFinite(planedHour) && planedHour > 0 ? planedHour : null,
...
export function validateDashForm(input) {
  ...
  const planedHour = Number(input?.planed_hour);
  ...
  if (!Number.isFinite(planedHour) || planedHour <= 0)
    errors.planed_hour = 'Planned hours must be a number greater than 0.';
      ...
      .upsert(
        {
          ...
          planed_hour: values.planed_hour,
...
export function createDashRowsFetcher(supabaseClient) {
  return async function fetchDashRows({ date } = {}) {
    let query = supabaseClient
      .from(DASH_TABLE)
      .select('id,department,date,planed_qty,planed_hour,eficiancy,available_man_power')
